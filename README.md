...
- 📱 **Cyber Gain H5 独立站** - 像素级移动端游戏大厅（500px 最大宽度）
...
- 🔒 **安全防护** - AES-256-GCM 数据加密、IP 白名单、速率限制、完整审计日志
...
│   │   └── package.json               # 依赖配置
│   │
│   ├── cyber-gain-web/                # Cyber Gain H5 独立站 (@flipcoin/cyber-gain-web)
│   │   ├── src/
│   │   │   ├── components/            # Vue 组件
│   │   │   │   ├── common/            # 通用组件
│   │   │   │   │   └── Banner.vue           # 轮播横幅组件
│   │   │   │   ├── home/              # 首页组件
│   │   │   │   │   ├── HomeTabs.vue         # 主选单 Tab（首页、哈希游戏、体育等）
│   │   │   │   │   └── GameSection.vue      # 游戏分类区块（带动态滑动遮罩）
│   │   │   │   ├── layout/            # 布局组件
│   │   │   │   │   └── Header.vue           # 顶部导航栏
│   │   │   │   └── ui/                # UI 组件
│   │   │   │       └── GameCard.vue         # 游戏卡片组件（支持 lg/md 尺寸）
│   │   │   ├── views/                 # 页面视图
│   │   │   │   └── Home.vue           # 首页（Banner + Tabs + 游戏分类）
│   │   │   ├── router/                # 路由配置
│   │   │   │   └── index.js           # Vue Router 配置
│   │   │   ├── styles/                # 样式文件
│   │   │   │   ├── main.css           # 主样式文件（Tailwind + Design Tokens）
│   │   │   │   └── tokens.css         # Design System tokens
│   │   │   └── App.vue                # 根组件（500px 最大宽度居中布局）
│   │   ├── Dockerfile.dev             # 开发环境 Docker 构建（支持热重载）
│   │   ├── Dockerfile                 # 生产环境 Docker 构建
│   │   ├── vite.config.js             # Vite 配置（port 3001）
│   │   ├── tailwind.config.js         # Tailwind CSS 配置
│   │   ├── postcss.config.js          # PostCSS 配置
│   │   ├── nginx.conf                 # Nginx 配置（生产部署）
│   │   ├── package.json               # 依赖配置
│   │   ├── README.md                  # Cyber Gain Web 专属文档
│   │   ├── HOME_IMPLEMENTATION.md     # 首页实现详细文档
...
│       │   ├── encryptionUtils.js     # 🔐 AES-256-GCM 加密工具（2FA、PII 加密）
│       │   ├── maskUtils.js           # 敏感数据脱敏（地址、Hash、Email、IP、Device ID）
...
│       │   ├── migrate-2fa-secrets.js # 🔐 2FA 密钥加密迁移脚本（Phase 1）
...
│   │   │   ├── add_encrypted_2fa.sql  # 🔐 添加 2FA 加密字段（Phase 1, 2026-02-12）
...
- **设计系统** - Design Tokens（颜色、间距、圆角）
- **响应式布局** - 500px 最大宽度，完美适配移动端
...
### 数据加密与密钥管理
- ✅ **2FA Secret 加密** - Google Authenticator 密钥使用 AES-256-GCM 加密存储（Phase 1）
- ✅ **密码加密** - 使用 bcrypt (cost=10) + SHA-256 指纹防重复密码
- ✅ **HD 钱包管理** - BIP44 标准派生，私钥不存数据库（KmsService）
- ✅ **环境变量保护** - 生产环境强制使用云平台环境变量服务（AWS/GCP/Azure）

### 访问控制与认证
- ✅ **JWT 身份验证** - 无状态 Token 认证（8 小时有效期）
- ✅ **谷歌二次验证（2FA）** - 管理员可启用 Google Authenticator（TOTP window: 1，90秒有效期）
- ✅ **IP 白名单** - 后台管理需通过 IP 白名单验证
- ✅ **速率限制** - 注册、登录、提款接口限流
- ✅ **并发控制** - 余额更新使用 `SELECT ... FOR UPDATE NOWAIT` 行锁
...
### 审计与日志
- ✅ **完整审计日志** - 登入成功/失败、关键操作全记录（Phase 1 强化）
- ✅ **敏感数据遮罩** - 12 种类型遮罩（Email、IP、Device ID、地址、Hash 等）
- ✅ **日志分级管理** - ERROR/WARN/INFO/DEBUG，生产环境禁用 DEBUG
- ✅ **失败登入记录** - 暴力破解攻击检测与追踪（Phase 1 新增）

### API 安全
- ✅ **响应标准化** - 统一错误处理，不暴露内部错误（Stack Trace）
- ✅ **输入验证** - 注册、登录、提款等接口严格验证
- ✅ **中文错误消息** - 所有 API 响应使用简体中文（符合宪法规范）
...
# - JWT_SECRET: JWT 密钥
# - ENCRYPTION_KEY_2FA: 2FA 密钥加密密钥（64 位十六进制字符串）
...
- **Cyber Gain H5**: http://localhost:3001
...
Cyber Gain H5 将在 `http://localhost:3001` 启动

**特性**:
- ✨ **像素级还原设计** - 严格遵循 500px 移动端布局
- 🎨 **动态滑动遮罩** - 智能检测滚动位置，左右漸层自适应
- 🎮 **多尺寸卡片** - 热门游戏（120px lg）+ 其他分类（110px md）
- 💎 **金色光晕** - 热门游戏专属边框与阴影效果
- 🔄 **热重载支持** - Docker 开发模式下修改代码即时刷新
...
### 安全与加密 🔐
- [2FA 加密部署指南](./apps/backend-legacy/docs/2FA_ENCRYPTION_GUIDE.md) - Phase 1 部署完整指南
- [日志规范指南](./apps/backend-legacy/docs/LOGGING_GUIDE.md) - 日志遮罩与审计规范
...
1. **环境变量**: 确保正确配置 `.env` 文件
   ...
   - `ENCRYPTION_KEY_2FA`: 2FA 密钥加密密钥（64 位 hex，生产环境必须使用 `openssl rand -hex 32` 生成）
   - `DATABASE_URL`: 数据库连接字符串
...
1. **安全强化** (Phase 1, 2026-02-12):
   - 🔐 2FA Secret AES-256-GCM 加密
   - 📋 完整的失败登入审计日志
   - 🌏 所有 API 错误消息中文化
   - 🔒 TOTP 时间窗口强化（150s → 90s）
2. **API 响应标准化**: 所有 API 统一使用 `{ success: true, data: ... }` 格式
3. **路由模块化**: 
   - `server.js` 路由拆分 → `routes/v1/` (前台 API)
   - `admin.js` 路由拆分 → `routes/admin/` (后台 API)
4. **服务层抽象**: 创建 `UserService`、`AdminService`、`GameService`、`WithdrawalService`
5. **前台 API 适配**: 修复所有前台 API 响应处理，适配标准格式
6. **充值地址显示**: 修复字段名匹配问题（`tron_deposit_address`、`evm_deposit_address`）
7. **并发控制**: 实现用户注册时的索引分配并发控制（`SELECT ... FOR UPDATE NOWAIT`）
8. **区块同步**: 实现区块高度持久化（`blockchain_sync_status` 表）
9. **归集重试**: 实现归集失败重试机制（`collection_retry_queue` 表）
10. **前端重构**: 从旧版单文件架构重构为 Vue 3 SPA 架构
11. **Monorepo 架构**: 使用 pnpm workspace 管理多个应用和包
...
Cyber Gain H5 是一个专为移动端优化的游戏大厅独立站，采用像素级设计还原，提供流畅的用户体验。

### 核心特性

- ✨ **像素级设计还原** - 严格遵循 500px 最大宽度移动端布局
- 🎨 **智能动态遮罩** - 横向滚动时左右渐层自适应显示
- 🎮 **多尺寸卡片系统** - 热门游戏（120px lg）+ 普通游戏（110px md）
- 💎 **金色光晕效果** - 热门游戏专属边框（1.5px）与阴影
- 🏷️ **精致角标设计** - 极致压缩的"热门"标签（9px 字体）
- 📐 **Design Tokens** - 统一的颜色、间距、圆角系统
- 🔄 **热重载支持** - Docker 开发环境下修改即时生效
...
├── src/
│   ├── components/
│   │   ├── common/Banner.vue          # 轮播横幅
│   │   ├── home/
│   │   │   ├── HomeTabs.vue          # 主选单 Tab（固定 37px 高度）
│   │   │   └── GameSection.vue       # 游戏分类区块
│   │   ├── layout/Header.vue         # 顶部导航
│   │   └── ui/GameCard.vue           # 游戏卡片（支持 lg/md）
│   ├── views/Home.vue                # 首页组合
│   ├── router/index.js               # 路由配置
│   └── styles/
│       ├── main.css                  # Tailwind 主样式
│       └── tokens.css                # Design Tokens
├── vite.config.js                    # port 3001 配置
...
- **Tab 高度**: 37px（固定）
- **卡片尺寸**:
  - 热门游戏（lg）: 120px × 172px（比例 120/172）
  - 普通游戏（md）: 110px × 154px（比例 110/154）
- **卡片间距**: 8px（gap-2）
- **区块间距**: 32px（mb-8）
...
#### 视觉效果
- **金色边框**: `border-[1.5px] border-[#EAB308]/80`
- **光晕阴影**: `shadow-[0_0_12px_rgba(234,179,8,0.3)]`
- **星星 3D**: `drop-shadow-md`
- **角标渐层**: `from-[#E11D48] to-[#EF4444]`
...
1. **严格遵循 flip-coin-rules.mdc** - 所有代码必须符合项目宪法规范
2. **使用 Vue 3 Composition API** - 必须使用 `<script setup>` 语法
3. **Tailwind CSS 优先** - 避免内联样式，使用 Design Tokens
4. **简体中文注释** - 所有注释和文档使用简体中文
5. **像素级精度** - 尺寸、间距、颜色严格按设计稿实现

### 文档参考

- [详细实现文档](apps/cyber-gain-web/HOME_IMPLEMENTATION.md)
- [项目初始化报告](apps/cyber-gain-web/INIT_REPORT.md)
- [Cyber Gain Web README](apps/cyber-gain-web/README.md)
